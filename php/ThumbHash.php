...
class ThumbHash
{
  ...
  public static function decodeHash($hash) {
    return array_map("ord", str_split(base64_decode($hash . "=")));
  }

  public static function rgbaToThumbHash($w, $h, $rgba) {
    // Encoding an image larger than 100x100 is slow with no benefit
    if ($w > 100 || $h > 100) {
      throw new Exception("$w x $h doesn't fit in 100 x 100");
    }
  ...
  public function rgbaToDataURL($w, $h, $rgba) {
    ...
    // png end
    $bytes = array_merge($bytes, [ $b >> 8, $b & 255, $a >> 8, $a & 255,  0,  0,  0,  0,  0,  0,  0,  0,  73,  69,  78,  68,  174,  66,  96,  130 ]);
